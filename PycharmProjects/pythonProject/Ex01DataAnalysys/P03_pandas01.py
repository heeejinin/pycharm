...
'''
# 1. Pandas의 자료형
#   1)Series 자료형 => 보통 1차원일 때 씀 / 인덱스와 값을 가짐
#       • 인덱스와 값을 가지고 있음
#       • 별도로 인덱스, 값을 출력할 수있음
...
#   2)DataFrame 자료형 => 주로 다차원에서 씀 / 행과 열로 이루어져 있다.
# • 행과 열로 이루어진 자료형
...
import numpy as np
zero_data = np.zeros(len(unique_genre)) #np.zeros: 모두 0으로 채움
...
import matplotlib.pyplot as plt #matplotlib: 데이터를 시각화 하기 위해 사용
